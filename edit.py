...
def updateResult(updated, new_text_list,edit_log):
	field = ['Analysis Name: ', 'Description: ']
	if updated == True:
		edit_log.log_write('---------------------------------------------------\n')
...
if __name__ == '__main__':
	...
	# edit 
	edit = editAnalysis(browser,web_dict,edit_fields,edit_text,web_dict['edit_update_css'])
	...
	# edit 
	edit = editAnalysis(browser,web_dict,edit_fields,edit_text,web_dict['edit_cancel_css'])
	updateResult(edit,edit_text,edit_log)

	# go to 'analyst result'
	result = browser.find_element_by_xpath('//*[@id="ember776"]').click()
	time.sleep(2)
	browser.find_element_by_xpath('//*[@id="ember1010"]').click() # click the 'i'
	clickIcon(browser,web_dict['edit_analysis_icon'])
	edit = editAnalysis(browser,web_dict,edit_fields,edit_text,web_dict['edit_update_css'])
	updateResult(edit,edit_text,edit_log)
	edit_log.log_close()

	goPage(browser,web_dict['home_page2'])
